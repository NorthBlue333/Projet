...
def spawn_bubbles(canvas, bubbles):
    ...
    if spawn_prob >= 0.99:
        ...
        bubbles.append(Bubble(x1, y1, random.randint(0, 9), x2=x1 + 40, y2=y1 + 40, tag="bubbles"))
        print(bubbles[0].number)
    return bubbles
...
def move_ball(canvas, ball, paddles, scores, bubbles):
    ...
    if ball.start:
        ...
        if won == False:
            ...
            won = False
            canvas.after(2000, move_ball, canvas, ball, paddles, scores, bubbles)
    else:
        if len(bubbles) < 1:
...
def change_players(nbplayers, looser, canvas):
    ...
    canvas.create_text(width/2, height/2, text=loose, font=("Helvetica", 32), fill=colors[color].foreground, tag="mid")
    if change != None:
        canvas.create_text(width/2, height/2 + 50, text=change, font=("Helvetica", 25), fill=colors[color].foreground, tag="mid")
    canvas.after(2000, canvas.delete, "mid")
...
def get_game_frame(master):
    ...
    for i in range(players.get()):
        if i == 0:
            paddles.append(Paddle(23, height / 2, sizex=15, sizey=160, sy=30, color=colors[color].foreground, outline=colors[color].foreground, tag="paddleL"))
        ...
        if i == 1:
            paddles.append(Paddle(width - 23, height / 2, sizex=15, sizey=160, sy=30, color=colors[color].foreground, outline=colors[color].foreground, tag="paddleR"))
        ...
        if i == 2:
            paddles.append(Paddle(width / 2, 23, sizex=160, sizey=15, sx=15, color=colors[color].foreground, outline=colors[color].foreground, tag="paddleT"))
        ...
        if i == 3:
            paddles.append(Paddle(width / 2, height - 23, sizex=160, sizey=15, sx=15, color=colors[color].foreground, outline=colors[color].foreground, tag="paddleD"))
